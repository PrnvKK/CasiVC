...
class TrainingConfig:
  ...
  classifier_weight: float = 0.3         # reduced from 1.0: CE feedback loop was exploding FiLM features. 0.3 is the stable value from Session 2/3 — L1 (5.0) structurally dominates CE gradient, FiLM only amplifies when it genuinely improves reconstruction.
  mel_classifier_weight: float = 0.0   # disabled: per-frame mel Conv1d CE caused upstream regression
  pooled_mel_ce_weight: float = 0.6    # reduced from 2.0: paired with classifier_weight reduction to prevent CE arms race

...
class PositionAgnosticCrossAttention(nn.Module):
    ...
    def forward(self, content_features, speaker_features, return_attention=False):
        ...
        if v and self.enable_residual:
            print(f"[cross_attn] T={temperature.item():.3f} ent={entropy:.2f} "
                  f"att_σ={attended_features.std():.3f} "
                  f"γ_μ={gamma.mean():.2f} γ_σ={gamma.std():.2f} "
                  f"r/FiLM={residual_norm.std()/film_output.std():.2f}")
        elif v:
            print(f"[cross_attn] T={temperature.item():.3f} ent={entropy:.2f} "
                  f"att_σ={attended_features.std():.3f}")

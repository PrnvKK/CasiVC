...
class MobileNetDecoder(nn.Module):
    ...
    def __init__(self, config: ModelConfig, return_feats: bool = False):
        ...
        # Reverted T1 — adapter is a learnable rotation again
        _adapter_conv = nn.Conv1d(in_channels, channel_progression[0], kernel_size=1, bias=True)
        nn.init.xavier_uniform_(_adapter_conv.weight)
        ...
        # SpeakerDeltaProj(96→80) for spk_film CE, reads per-frame x + speaker tokens.
        #   Per-frame structure (reads x.detach()) breaks the time-invariance ceiling
        #   that capped mel_proj_speaker (RMS 0.036). x.detach() blocks CE from
        #   reaching decoder blocks — CE trains only SpeakerDeltaProj.mlp.
    ...
    def forward(self, fused: torch.Tensor, return_intermediate: bool = False, speaker_feats: torch.Tensor | None = None, content_speaker_feats: torch.Tensor | None = None) -> Tuple[torch.Tensor, List[torch.Tensor]] | torch.Tensor:
        ...
        # Speaker path (SpeakerDeltaProj): reads per-frame x + speaker tokens — CE only.
        #   x.detach() blocks CE from reaching decoder blocks — CE trains only
        #   SpeakerDeltaProj.mlp. Per-frame x gives temporal structure that breaks
        #   the time-invariance ceiling of mel_proj_speaker (RMS 0.036).
        #   L1 Path 3 still reaches SpeakerDeltaProj.mlp via pred_mel → spk_film_mel,
        #   but SpeakerDeltaProj has per-frame capacity to satisfy both L1 and CE
        #   (unlike the time-invariant Conv1d that L1 pushed to near-zero).
        ...
        prebias_mel = content_mel  # [B, 80, T] — RAW, before any scaling
        ...
        mel_pre_scale_std = content_mel.std().item()
        mel_pre_scale_mean = content_mel.mean().item()
        ...
        #   L1 on pred_mel    → trains SpeakerDeltaProj via mel_speaker (Path 3)
        #   CE on mel_speaker → trains SpeakerDeltaProj for speaker discrimination
        ...
        clamp_mask = (spk_film_mel < -11.5) | (spk_film_mel > 2.0)
        ...
        if should_return_feats:
            intermediate.append(prebias_mel)   # [-4] raw mel (pre-scale) — L1 target
            intermediate.append(variance_mel)  # [-3] post-scale, pre-affine — variance target
            intermediate.append(mel_speaker)   # [-2] speaker delta — spk_film CE target
